...
if (isset($_REQUEST['action'])) {
     ...
     ///deletePack
     if ($_REQUEST['action'] == 'deletePack') {
          $productId = mysqli_real_escape_string($link, $_REQUEST['productId']);
          $passportIn = $cal->selectFrmDB($starter_pack_tb, 'product_thumbnail', 'product_id', $productId);
          $passportIn2 = $cal->selectFrmDB($starter_pack_tb, 'product_image', 'product_id', $productId);
          if (!empty($passportIn)) {
               $bassic->unlinkFile($passportIn, '../../photo/');
          }
          if (!empty($passportIn2)) {
          ...
          if (query_sql("DELETE FROM $starter_pack_tb WHERE `product_id`='" . $productId . "' LIMIT 1")) {
               $msg = 'Product was successfully deleted!';
               echo json_encode(array('success' => 'Successful Update!!!', 'msg' => $msg));
          } else {
               $msg = 'Product failed to delete!';
