...
if (isset($_POST['sub'])) {
 ...
 if (!empty($product_name) && !empty($product_category) && !empty($product_description) && !empty($product_price)) {
  ...
  $passportIn1 = $cal->selectFrmDB($product, 'product_image', 'product_id', $productId);
  ...
  if (!empty($pic_name) && !empty($pic_name1)) {
   if (empty($picvalidation)) {
    ...
    if (empty($upl)) {
     ...
     $update = $cal->update($product, $fieldup, $valueup, 'product_id', $productId);
     if (!empty($update)) {
      $msg = $update;
     } else {
...
$sql = query_sql("SELECT * FROM $product WHERE `product_id`='" . $productId . "' ORDER by id LIMIT 1");
       ...
       <form method="post" enctype="multipart/form-data">
        <div class="row">
         ...
         <div class="col-12">
          <div class="card">
           <div class="card-header">
            <h5>Product Images (Note: Ensure to select both images to enable update)</h5>
           </div>
           <div class="card-body">
            <div class="input-items">
             <div class="row gy-3">
